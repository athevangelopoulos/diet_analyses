...
diet_proportions.R computes the %F, %N, %W and %IRI indices for the input data and exports index values as a .csv file. The %N, %W and %IRI values for the prey taxa are also plotted as pie-donut graphs and exported as .png files. pie-donut graphs present in a single plot the proportions of prey taxa individually in the outer "donut" part and aggregated into categories in the inner "pie" part of the plot.
